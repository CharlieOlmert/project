...
# black for miss, red for save). Two different maps for shots that satisfy the new 60-second
# shot clock rule versus shots that fail the new rule.

field_plot <- ggplot(brown_game, aes(x = sh_x_pxls, y = sh_y_pxls, color = result)) + 
  annotation_custom(field, -250, 250, -50, 420) +
  geom_point(stat = "identity", position = "dodge", alpha = 0.5) +
  xlim(-250, 250) +
  ylim(-50, 420) +
  facet_wrap(~ sh_clock, ncol = 1, labeller=labeller(sh_clock = labels)) +
